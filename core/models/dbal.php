...
class dbal implements dbalInterface {
    ...
    public function myQuery($query = null) {
        $data = array();
        if (!empty($query)) {
            $result = mysqli_query(database::dbConnection(), $query);
    ...
    private function executeQuery($query) {
        ...
        if (mysqli_query(database::dbConnection(), $query)) {
            $this->last_inserted_id = database::dbConnection()->insert_id;
            return TRUE;
        } else {
            return "Error: " . $query . "<br>" . mysqli_error(database::dbConnection());
        }             
    }    
